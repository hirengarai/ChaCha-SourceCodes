...
int main()
{
    ...
    while (loop < 1024 * 1024)
    {
        ...
        while (1)
        {
            ...
            uint32_t Z1 = extractBits(z1[j1], 6);
            uint32_t X = extractBits(x[j1], 6);
            uint32_t Z2 = extractBits(z2[j1], 6);
            uint32_t X1 = extractBits(x1[j1], 6);

            if (
                /*for b part*/ (Z1 > X) && (Z2 > X1) &&
