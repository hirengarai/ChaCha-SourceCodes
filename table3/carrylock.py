...
for _ in range(num_trials):
    ...
    while (Z & bit_mask_pre_pnb_block) < (X & bit_mask_pre_pnb_block):
        Z = random.getrandbits(32)
        X = random.getrandbits(32)
    
    Z = (Z & ~bit_mask_pnb_block) | (0b11111 << pnb_block[0]) 
    
    # With 50% chance, modify X's PNB block
    if random.getrandbits(1):
        X1 = (X & ~bit_mask_pnb_block) | (random.getrandbits(len(pnb_block)) << pnb_block[0])
    else:
        X1 = X
