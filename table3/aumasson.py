"""
EXPECTED CARRY PROPAGATION PAST A PNB BLOCK -- AUMASSON et al. APPROACH

Synopsis:
Estimates E[X], the number of bit positions above a PNB block [n2:n1] at
which the modular differences (Z - X) and (Z - X1) disagree, where X1 is X
with its PNB block randomised. No condition is imposed on the surrounding
bits, which is the baseline approach of Aumasson et al. (FSE 2008).
...
def main() -> int:
    ...
    if args.json:
        ...
        record.update({"method": "aumasson", "expected_propagation": result,
                       "runtime_s": runtime})
