/*
 * Filename: pnb_search_xor_condition.cpp
 *
 * Synopsis:
 * Multi-threaded PNB (Probabilistic Neutral Bits) search for ChaCha using
 * the XOR shortcut. For each key bit, runs the forward / backward pipeline
 * with XOR replacing modular +/- (sumstate = X XOR X^R, minusstate =
 * sumstate XOR (X^R with the bit flipped)), measures the parity bias, and
 * classifies bits whose |bias| >= threshold as PNBs.
 *
 * CLI:
 *   ./pnb_search_xor_condition [neutrality_threshold]
 *   Example: ./pnb_search_xor_condition 0.35
 */
...
constexpr double DEFAULT_NEUTRALITY_THRESHOLD = 0.35;
...
int main(int argc, char *argv[])
{
    ...
    cout << chacha::ChaChaTraits<TargetWord>::name << " PNB Search (XOR mode)\n";
    ...
    cout << "Mode                     : XOR (sum/minus replaced by XOR)\n";
    cout << report_util::SEP << "\n";
...
double matchcount(int key_bit, int key_word)
{
    ...
    for (size_t loop = 0; loop < SAMPLES.samples_per_thread_; ++loop)
    {
        ...
        // XOR shortcut for sumstate (forward).
        state_ops::xor_state(x0,  strdx0,  sumstate);
        state_ops::xor_state(dx0, dstrdx0, dsumstate);

        // Flip the target key bit (mirror to k[key_word+4] for 128-bit).
        ...
        // XOR shortcut for minusstate (backward).
